<script>
	import CallUs from '../lib/callus/CallUs.svelte';
</script>

<img
	class="sticky top-24 -z-10"
	src="/img/antifurti-2f-sicurezza-treviglio-003-1920w.webp"
	alt="sistemi antintrusione"
/>
<CallUs />
<div class="p-5 text-center bg-2f-blue-200">
	<div class="font-['Oswald'] tracking-wide text-3xl uppercase">
		Vendita telecamere di videosorveglianza con tecnologia IP in Lombardia
	</div>
	<p class="text-base leading-relaxed pt-2">
		2F Sicurezza, a Treviglio, è un'azienda leader nella realizzazione di sistemi di
		videosorveglianza con tecnologia IP. Con il termine videosorveglianza si intende l'attività del
		vigilare un luogo a distanza o direttamente, tramite l'uso di telecamere o di altri strumenti in
		grado di assicurare la trasmissione di immagini strategicamente posizionate. Adottare il più
		efficace impianto di videosorveglianza significa mettere in piena sicurezza qualsiasi bene
		grazie al vantaggio di poterlo controllare facilmente anche quando non si è all'interno della
		struttura protetta. 2F Sicurezza propone sistemi di videosorveglianza con l'uso di tecnologia IP
		consentendo a chi ne usufruisce di controllare la propria casa o la propria azienda tramite
		internet, smartphone ed altri dispositivi.
	</p>
</div>
<div class="grid grid-cols-1 bg-gray-100">
	<div class="p-5 text-start">
		<div class="font-['Oswald'] tracking-wide text-center text-3xl">Domotica intelligente</div>
		<p class="text-base leading-relaxed pt-2">
			2F Sicurezza opera nel campo della domotica fornendo soluzioni integrate di videosorveglianza
			grazie all'uso della tecnologia IP per un controllo assolutamente sicuro della propria
			abitazione o del proprio luogo di lavoro. Il mondo della videosorveglianza offre, ad oggi,
			quattro grandi tipologie di tecnologie:
		</p>
		<ul class="list-disc list-inside p-3">
			<li>Analogica</li>
			<li>Hd-Cvi</li>
			<li>Ahd</li>
			<li>Ip</li>
		</ul>
		<p>
			La prima è una tecnologia ormai superata con telecamere che arrivano ad una risoluzione
			massima di 960x576 (1000 Tvl). La tecnologia Hd-Cvi, brevettata da Dahua, tramite cavo
			coassiale, consente la trasmissione fino a lunghe portate (più o meno 500 MT) del segnale
			video in risoluzione 1080P (1920x1080) di alta qualità. La tecnologia Ahd (Analogic High
			Definition) permette una risoluzione che passa dall'Hd (1280x720) fino al Full Hd (19220x1080)
			offrendo compatibilità con quasi tutti i Dvr (Digital Video Recorer).La tecnologia IP è,
			invece, caratterizzata da una telecamera che genera un segnale digitalizzato che può essere
			subito immesso in rete.
		</p>
	</div>
	<div class="p-5 text-start">
		<div class="font-['Oswald'] tracking-wide text-center text-3xl">
			I vantaggi delle telecamere IP
		</div>
		<p class="text-base leading-relaxed pt-2">
			La tecnologia IP nella videosorveglianza consente non solo una risoluzione molto superiore
			all'uso delle classiche telecamere analogiche che può arrivare anche a 12MP (mega pixel), con
			segnali digitali che possono essere subito immessi in rete ed archiviati. Inoltre presenta
			vantaggi notevoli quali:
		</p>
		<ul class="list-disc list-inside p-3">
			<li>Analogica</li>
			<li>Hd-Cvi</li>
			<li>Ahd</li>
			<li>Ip</li>
		</ul>
		<p>
			2F Sicurezza propone ai propri clienti telecamere con tecnologia IP: le più avanzate sul
			mercato.
		</p>
	</div>
</div>
<div class="p-5 text-center bg-2f-blue-200">
	<div class="font-['Oswald'] tracking-wide text-3xl uppercase">I nostri lavori</div>
	<div class="pt-2 flex overflow-x-scroll gap-5">
		<img src="/img/lavori/HelloIMG1655222543305.jpeg" alt="i nostri lavori 1" />
		<img src="/img/lavori/HelloIMG1655222558339.jpeg" alt="i nostri lavori 2" />
		<img src="/img/lavori/HelloIMG1655222567280.jpeg" alt="i nostri lavori 3" />
		<img src="/img/lavori/HelloIMG1655222576481.jpeg" alt="i nostri lavori 4" />
		<img src="/img/lavori/HelloIMG1655222581308.jpeg" alt="i nostri lavori 5" />
		<img src="/img/lavori/HelloIMG1655222586405.jpeg" alt="i nostri lavori 6" />
		<img src="/img/lavori/HelloIMG1655222596066.jpeg" alt="i nostri lavori 7" />
		<img src="/img/lavori/HelloIMG1655222596572.jpeg" alt="i nostri lavori 8" />
		<img src="/img/lavori/HelloIMG1655222596944.jpeg" alt="i nostri lavori 9" />
		<img src="/img/lavori/HelloIMG1655222597192.jpeg" alt="i nostri lavori 10" />
		<img src="/img/lavori/HelloIMG1655222597443.jpeg" alt="i nostri lavori 11" />
		<img src="/img/lavori/HelloIMG1655222597687.jpeg" alt="i nostri lavori 12" />
		<img src="/img/lavori/HelloIMG1655222597930.jpeg" alt="i nostri lavori 13" />
		<img src="/img/lavori/HelloIMG1655222599346.jpeg" alt="i nostri lavori 14" />
		<img src="/img/lavori/HelloIMG1655222599816.jpeg" alt="i nostri lavori 15" />
		<img src="/img/lavori/HelloIMG1655222600061.jpeg" alt="i nostri lavori 16" />
		<img src="/img/lavori/HelloIMG1655222600304.jpeg" alt="i nostri lavori 17" />
		<img src="/img/lavori/HelloIMG1655222600548.jpeg" alt="i nostri lavori 18" />
		<img src="/img/lavori/HelloIMG1655222601064.jpeg" alt="i nostri lavori 19" />
		<img src="/img/lavori/HelloIMG1655222601309.jpeg" alt="i nostri lavori 20" />
		<img src="/img/lavori/HelloIMG1655222601555.jpeg" alt="i nostri lavori 21" />
	</div>
</div>
